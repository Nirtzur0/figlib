...
figures/<subject>/*.py
                 ...
                 infotheory, circuits.
figures/out/<subject>/
                 COMMITTED render baselines: `<name>.{svg,png}` on the theme's
                 the theme's stock (the default) and `<name>_transparent.{svg,png}` on
                 alpha, plus readback/judge records. Golden files `--regress` diffs against, not scratch.
...
docs/            everything above; docs/brand/ is the wordmark and social
                 card, drawn by the library (`make brand`)
```
...
- **A figure is a printed page, and grain is ink.** `Style.transparent`
  defaults to `False`: the theme's paper stock, its grain, and label casings
  ride every render. **Grain is ink, not paper — it renders on either ground.**
  Every figure commits BOTH — `<name>.svg` on stock (the default),
  `<name>_transparent.svg` on alpha for a document that owns its own
  background. `regress.variants` returns both unconditionally, so a figure
  cannot quietly stop being checked against the ground its contrast gate
  assumes.
