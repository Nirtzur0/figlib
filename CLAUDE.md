...
figures/out/<subject>/
                 ...
                 cream (the default) and `<name>_transparent.{svg,png}` on
...
- **A figure is a printed page, and grain is ink.** `Style.transparent`
  defaults to `False`: the theme's cream paper, its grain, and label casings
  ride every render. **Grain is ink, not paper — it renders on either ground.**
  Every figure commits BOTH — `<name>.svg` on cream (the default),
