...
The drawn equation is EXACT: the left side is K_4, the rank-4 truncation,
not K. What the truncation throws away is admitted on the figure as a
number rather than hidden in a caption. The successive modes are SIGNED
and oscillate at rising frequency, so every block shares one symmetric
color scale; the Hinton inset carries the sign structure directly, which
no monotone ramp can.
"""
...
CLAIM = (
    "A smooth Gaussian kernel matrix is nearly rank 4: the drawn sum is "
    "exactly its rank-4 SVD truncation and keeps 99.7% of the Frobenius "
    "energy, the discarded 5.6% relative error is stated on the figure "
    "rather than hidden, and the successive rank-1 modes are signed and "
    "oscillate at rising frequency, like a Fourier basis."
)
...
PARAMS = {
    ...
    "bar_len": 1.30,        # colorbar length, math units
    "bar_thick": 0.14,
    "pad": 0.30,
}
...
def compute(p):
    ...
    return {"A": A, "A_r": A_r, "terms": terms, "s": s, "U": U, "Vt": Vt,
...
def _row(g):
    p = g["params"]
    terms = [Block(*g["A_r"].shape, values=g["A_r"], name="K_4"), "="]
...
def build(g):
    ...
    vmax = max(float(np.abs(T).max()) for T in [g["A_r"], *g["terms"]])

    s = Scene()
    for k, b in enumerate(placed):
        s.add(heat(b, ramp=THEME.ramp, vmin=-vmax, vmax=vmax))
        s.add(*outline(b, width_scale=WEIGHT_BG if k == 0 else WEIGHT_ACTOR))
        cap = (r"K_4" if k == 0
    ...
    s.add(MathLabel(
        r"\boldsymbol{v}_1..\boldsymbol{v}_4\ \mathrm{(area=|v|,\ fill=sign)}",
    ...
    s.add(*colorbar(bar_scale, THEME.ramp,
                    at=(placed[-1].extent[1] + 0.30, hb.extent[2]),
                    length=p["bar_len"], thickness=p["bar_thick"],
                    orient="y",
                    ticks=Ticks(values=tv, positions=bar_scale.fwd(tv),
                                labels=("-", "0", "+")),
                    label=r"\mathrm{mode\ value}"))
    ...
    s.xlim = (xlim[0], xlim[1] + 1.5)          # room for the inset legend
